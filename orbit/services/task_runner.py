...
class TaskRunner:
    ...
    async def execute_workflow(self, workflow_id: UUID) -> None:
        ...
        await self.ws_manager.broadcast({"workflow_id": str(workflow_id), "status": "running"})
        ...
        try:
            ...
            self.session.add(workflow)
            await self.session.commit()
            await self.ws_manager.broadcast({"workflow_id": str(workflow_id), "status": "completed"})
    ...
    async def _execute_action(self, action_type: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        """
